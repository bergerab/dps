...
function fetchData(signals, startTime, endTime, samples, batch_process_id) {
  return new Promise(resolve => {
    ...
    console.log(startTime, util.dateToString(startTime),  endTime, util.dateToString(endTime));
    samples = samples || 10;
      api.post('get_chart_data', {
        series: signals.map(x => {
          ...
          if (batch_process_id !== undefined) { /* If a batch_process_id is provided, set the magic dataset name. */
            dataset = 'batch_process' + batch_process_id;
          }
        ...
        for (const s of data.datasets) {
          ...
          for (const d of s.data) {
            console.log('date from server', d.x);
